...
def get_heroes_in_draft():
    ...
    for hero, portrait in portraits:
        if (new_min := imagehash.average_hash(radiant_1) - imagehash.average_hash(portrait)) < min_radiant_1[1]:
            min_radiant_1 = (hero, new_min)
        if (new_min := imagehash.average_hash(radiant_2) - imagehash.average_hash(portrait)) < min_radiant_2[1]:
            min_radiant_2 = (hero, new_min)
        if (new_min := imagehash.average_hash(radiant_3) - imagehash.average_hash(portrait)) < min_radiant_3[1]:
            min_radiant_3 = (hero, new_min)
        if (new_min := imagehash.average_hash(radiant_4) - imagehash.average_hash(portrait)) < min_radiant_4[1]:
            min_radiant_4 = (hero, new_min)
        if (new_min := imagehash.average_hash(radiant_5) - imagehash.average_hash(portrait)) < min_radiant_4[1]:
            min_radiant_5 = (hero, new_min)
        if (new_min := imagehash.average_hash(dire_1) - imagehash.average_hash(portrait)) < min_dire_1[1]:
            min_dire_1 = (hero, new_min)
        if (new_min := imagehash.average_hash(dire_2) - imagehash.average_hash(portrait)) < min_dire_2[1]:
            min_dire_2 = (hero, new_min)
        if (new_min := imagehash.average_hash(dire_3) - imagehash.average_hash(portrait)) < min_dire_3[1]:
            min_dire_3 = (hero, new_min)
        if (new_min := imagehash.average_hash(dire_4) - imagehash.average_hash(portrait)) < min_dire_4[1]:
            min_dire_4 = (hero, new_min)
        if (new_min := imagehash.average_hash(dire_5) - imagehash.average_hash(portrait)) < min_dire_4[1]:
            min_dire_5 = (hero, new_min)


    radiant_draft = [min_radiant_1[0],min_radiant_2[0],min_radiant_3[0],min_radiant_4[0],min_radiant_5[0]]
    dire_draft = [min_dire_1[0],min_dire_2[0],min_dire_3[0],min_dire_4[0],min_dire_5[0]]
